...
- Out of `r format_count(df$n_valid[1])` valid OpenAPI version 2.x specifications, `r format_count_and_pct(df$n_has_host[1], df$pct_has_host[1])` have a `host` property, 
...
- Out of `r format_count(df$n_valid[1])` valid OpenAPI version 3.x specifications, `r format_count_and_pct(df$n_has_servers[1], df$pct_has_servers[1])` have a `servers` property, with an average of `r format(round(df$avg_server[1],1))` server/entry, and a range of `r format_count(df$min_servers[1])`-`r format_count(df$max_servers[1])`. 
