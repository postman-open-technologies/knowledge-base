...
- Out of the `r format_count(df$n_all[1])` server entries (across all APIs), `r format_count_and_pct(df$n_localhost[1], df$pct_localhost[1])` contain localhost or 127.0.0.1, `r format_count_and_pct(df$n_dnshost[1], df$pct_dnshost[1])` use a DNS host, and `r format_count_and_pct(df$n_templatehost[1], df$pct_templatehost[1])` use a variable template (3.x only). `r format_count_and_pct(df$n_relative[1], df$pct_relative[1])` use a relative path (no server specified). 
